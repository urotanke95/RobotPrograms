...
public class Square{
	...
	private static void trackGreen() throws AWTException{
		...
		JFrame frame2 = new JFrame("HSVVV");
				...
				if( !webcam_image.empty() )
				{
					...
					for (int idx = 0; idx != contours.size(); ++idx) {
						...
						if (contourarea > maxArea) {
							maxArea = contourarea;
							maxAreaIdx = idx;
							System.out.println("maxareaidx:"  + maxAreaIdx + ", MaxArea:" + maxArea);
						}
					}
					List<MatOfPoint> largest = new ArrayList<MatOfPoint>();
					largest.add(contours.get(maxAreaIdx));
					Imgproc.drawContours(webcam_image, largest, -1, new Scalar(255, 0, 0), 2);
